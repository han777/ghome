...
const newTemplate = `<template>
  <div class="order-detail-page" v-if="order">
    ...
    <div class="content">
      ...
      <!-- 1b. Stay Info Card (dates + order No) -->
      ...
      <div
        v-for="occupy in order.roomOccupies"
        :key="occupy.id"
        class="mobile-card room-occupy-card"
      >
        <div class="ro-row">
          <div class="ro-col">
        ...
        <div class="ro-occupants">
          ...
          <div class="occ-chip-row">
            ...
            <span
              v-for="(name, idx) in parseCoOccupants(occupy.coOccupants)"
              :key="idx"
              class="occ-chip"
            >{{ name }}</span>
          </div>
      ...
      <div v-if="order.productDetails && order.productDetails.length > 0">
        <div
          v-for="svc in order.productDetails"
          :key="svc.id"
          class="mobile-card service-card"
        >
    ...
    <div class="bottom-actions">
      ...
      <template v-else>
        <button
          ...
          @click="earlyCheckOut"
        >{{ $t('orderDetail.earlyCheckoutBtn') }}</button>
        <button
          class="action-btn"
          :disabled="![0, 1].includes(order.status)"
          @click="cancelBooking"
        >{{ $t('orderDetail.cancelBtn') }}</button>
      </template>
    </div>
  </div>
...
console.log('Template replaced. Total lines:', content.split('\n').length);
