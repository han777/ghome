...
const fetchData = async (page: number) => {
  ...
  let url = `/room-fee-detail/paged?page=${page}&size=${pageSize.value}`;
  ...
  try {
    ...
    console.error('Failed to fetch fee detail', e);
  }
};
...
const exportExcel = () => {
  let url = `/api/room-fee-detail/excel?`;
  if (checkInFrom.value) url += `checkInFrom=${checkInFrom.value}T00:00:00&`;
  if (checkInTo.value) url += `checkInTo=${checkInTo.value}T23:59:59&`;
  ...
  fetch(url, { headers: { Authorization: `Bearer ${token}` } })
    ...
    .then(blob => {
      ...
      a.download = 'room_fee_detail.xlsx';
      a.click();
