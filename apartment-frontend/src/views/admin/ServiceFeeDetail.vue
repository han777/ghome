      ...
      <table class="admin-table detail-table">
        ...
        <tbody>
          <tr v-for="(row, idx) in rows" :key="row.id">
            ...
            <td>¥{{ formatPrice(row.standardPrice) }}</td>
            <td>¥{{ formatPrice(row.actualPrice) }}</td>
            <td>{{ row.quantity }}</td>
...
const fetchData = async (page: number) => {
  ...
  let url = `/service-fee-detail/paged?page=${page}&size=${pageSize.value}`;
  if (checkInFrom.value) url += `&checkInFrom=${checkInFrom.value}T00:00:00`;
  if (checkInTo.value) url += `&checkInTo=${checkInTo.value}T23:59:59`;
  try {
    ...
    console.error('Failed to fetch service fee detail', e);
  }
};
...
const exportExcel = () => {
  let url = `/api/service-fee-detail/excel?`;
  if (checkInFrom.value) url += `checkInFrom=${checkInFrom.value}T00:00:00&`;
  if (checkInTo.value) url += `checkInTo=${checkInTo.value}T23:59:59&`;
  ...
  fetch(url, { headers: { Authorization: `Bearer ${token}` } })
    ...
    .then(blob => {
      ...
      a.download = 'service_fee_detail.xlsx';
      a.click();
