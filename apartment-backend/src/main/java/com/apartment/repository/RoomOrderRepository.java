...
public interface RoomOrderRepository extends JpaRepository<RoomOrder, Long> {
    ...
    @Query("SELECT COUNT(o) FROM RoomOrder o JOIN o.roomOccupies ro WHERE ro.room.id = ?1 AND o.status IN (1, 2) AND ro.status = 0 AND o.startDate < ?3 AND o.endDate > ?2")
    long countOverlappingActiveOrders(Long roomId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT o FROM RoomOrder o JOIN o.roomOccupies ro WHERE ro.room.id = ?1 AND o.status IN ?2 AND ro.status = 0")
    ...
    @Query("SELECT o FROM RoomOrder o JOIN o.roomOccupies ro WHERE ro.occupantUser.id = ?1 AND o.status IN ?2 AND ro.status = 0")
    java.util.List<RoomOrder> findByOccupantUserIdAndStatusIn(Long occupantUserId, java.util.Collection<Integer> statuses);
    
    @Query("SELECT DISTINCT ro.room.id FROM RoomOrder o JOIN o.roomOccupies ro WHERE o.status IN (1, 2) AND ro.status = 0 AND o.startDate < ?2 AND o.endDate > ?1")
