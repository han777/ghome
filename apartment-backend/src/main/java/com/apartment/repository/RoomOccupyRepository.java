...
public interface RoomOccupyRepository extends JpaRepository<RoomOccupy, Long> {
    ...
    // 查找当日入住的记录（用于生成日常保洁，仅已入住房间，且当日未退房）
    @Query("SELECT ro FROM RoomOccupy ro WHERE ro.status = 1 AND ro.checkInTime <= :endOfToday AND (ro.checkOutTime IS NULL OR ro.checkOutTime > :endOfToday)")
    ...
    // 查找当日退房的记录（用于生成强打扫，仅已入住且当日退房）
    @Query("SELECT ro FROM RoomOccupy ro WHERE ro.status = 1 AND ro.checkOutTime BETWEEN :startOfDay AND :endOfDay")
